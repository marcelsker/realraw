...
fn build(out_dir: &Path) {
    ...
    // Static link on Windows: headers default to dllimport without this.
    libraw.define("LIBRAW_NODLL", None);
    libraw.define("LIBRAW_NOTHREADS", None);
    ...
    // MSVC needs exception support for C++ streams; skip -pthread (GCC/Clang only).
    if libraw.get_compiler().is_like_msvc() {
        libraw.flag("/EHsc");
    } else {
